...
// assign input as template string
let input = 
...
// transform input 
let modinput = input.split('\n'); // convert string to array, sep by new line

modinput = modinput.map( x => {
    x = x.split(' '); // split individual entries into parts
    x[0] = x[0].split('-'); // split min max
    x[1] = x[1].replace(':', ''); // remove colon from target letter
    return x;
})
...
modinput.map( x => {
    let minmax = x[0];
    let target = x[1];
    let password = x[2].split('').filter( y => y ===  target ).length; // *** see below

    if (password >= minmax[0] && password <= minmax[1]){
        counter++;
    }
    return counter;
})
// *** converts password string to array of strings of individual characters. Then use filter() to shorten the string to the target character. Finally use .length to get the number of characters. E.g. "123" -> ["1", "2", "2"] -> filter for 2 ["2", "2"] -> length = 2
...
modinput.map( x => {
    ...
    let target = x[1];
    let password = x[2];
    // console.log(pos1, pos2, target, password);
    if (password[pos1] === target && password[pos2] !== target) {
        counter2++;
    } else if (password[pos1] !== target && password[pos2] === target) {
        counter2++;
    }
    return counter2;
})
